...
* utilize CATALYST package for IMC we can see scatter plots.
...
library(SingleCellExperiment)
 ...
 rd<-DataFrame(sample_id=colData(imc))
 ...
 pn<-percentilenormalize(data=t(assays(imc)$loglim),percentile=0.99 )

  ## Switching into Summarized Experiment class.
    dse<-SummarizedExperiment(assays=SimpleList(exprs=pn),
                           metadata=list(experiment_info=exper_info),
                           rowData=rd,
...
cluster<-rowData(dse)
 data<-data.frame(t(logcounts(imc)),cluster)
 plot_clustering_heatmap_wrapper(expr=data[,rownames(imc)],
                                 cell_clustering=data$cluster_id)
 
 ### sub-clustering 1 cluster.
 #dse_1<-dse[which(rowData(dse)$cluster_id==1),]
#  dse_1<-(generateClusters(dse_1,meta_clustering=TRUE,meta_k=30,seed_clustering=828))
#plot_clustering_heatmap_wrapper(expr=assays(dse_1)$exprs,
 #                                cell_clustering=rowData(dse_1)$cluster_id)
